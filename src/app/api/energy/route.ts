...
export async function GET(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({
      source: "EIA API v2",
      note: "EIA_API_KEY not configured. Register free at https://www.eia.gov/opendata/register.php",
      configured: false,
      results: [],
    });
  }
  ...
  try {
    const data = await cachedFetch(
      ...
      async () => {
        ...
        if (sort) params["sort[0][column]"] = "period";
        if (sort) params["sort[0][direction]"] = sort;

        // Parse filters: "stateid:US,sectorid:1" → facets
        if (filters) {
    ...
    return NextResponse.json({
      ...
      results: data.response.data.slice(0, parseInt(limit)),
    });
  } catch (err) {
